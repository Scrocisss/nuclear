...
bool authenticate_user(User *user) {
    ...
    char username[50];
    fgets(username, sizeof(username), stdin);  // Используем fgets вместо gets
    // Убираем символ новой строки, если он есть
    username[strcspn(username, "\n")] = '\0';

    char password[50];
    printf("Введите пароль: ");
    // Заменяем gets на fgets
    fgets(password, sizeof(password), stdin);  // Используем fgets вместо gets
    // Убираем символ новой строки, если он есть
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        authenticated = true;
        // Копируем данные из базы данных в структуру пользователя с использованием безопасных функций
        strncpy(user->username, (const char *)sqlite3_column_text(stmt, 0), sizeof(user->username) - 1);
        user->username[sizeof(user->username) - 1] = '\0';  // Обеспечиваем терминатор строки

        strncpy(user->password, (const char *)sqlite3_column_text(stmt, 1), sizeof(user->password) - 1);
        user->password[sizeof(user->password) - 1] = '\0';  // Обеспечиваем терминатор строки

        strncpy(user->role, (const char *)sqlite3_column_text(stmt, 2), sizeof(user->role) - 1);
        user->role[sizeof(user->role) - 1] = '\0';  // Обеспечиваем терминатор строки
    }
...
bool add_user(const char *username, const char *password, const char *role) {
    sqlite3 *db;
    sqlite3_stmt *stmt;
    const char *sql = "INSERT INTO users (username, password, role) VALUES (?, ?, ?);";  // Подготовленный запрос
    ...
    sqlite3_bind_text(stmt, 1, username, -1, SQLITE_STATIC);  // Привязка username
    sqlite3_bind_text(stmt, 2, password, -1, SQLITE_STATIC);  // Привязка password
    sqlite3_bind_text(stmt, 3, role, -1, SQLITE_STATIC);      // Привязка role
